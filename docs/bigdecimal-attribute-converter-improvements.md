...
> **Status: resolved as recommended.** C1 landed as the parallel numeric base class, C2 and C3 alongside it. What
> remains is step 4 of the sequencing table — flipping the default at the next major — which is recorded in
> `docs/MIGRATION-NEXT_MAJOR.md`. See [Resolution](#resolution) for what shipped.
...
|---|---|---|---|
| 1 | C2 — fix the javadoc and the LLM doc | none; do it now | done |
| 2 | C1 — parallel numeric base class and the two numeric converters, `autoApply = false` | decision on parallel-class vs documentation-only | done — parallel class chosen |
| 3 | C3 — migration note in `MIGRATION-NEXT_MAJOR.md` | follows step 2 | done |
| 4 | Next major: numeric becomes the default, double-backed classes deprecated `forRemoval` | a major | pending the major |
...
All four open questions were answered as recommended: the parallel class, both types moved, no imposed
precision or scale, and no minor-units mapping.

### Added

| File | Content |
|---|---|
| `converters/BaseBigDecimalTypeNumericAttributeConverter.java` | `AttributeConverter<T, BigDecimal>`; hands `value()` to JDBC unchanged in both directions |
| `converters/AmountNumericAttributeConverter.java` | `@Converter` (no `autoApply`) for `Amount` |
| `converters/PercentageNumericAttributeConverter.java` | `@Converter` (no `autoApply`) for `Percentage` |
| `test/.../converters/BigDecimalTypeNumericAttributeConverterTest.java` | 5 unit tests, no Docker |

The concrete converters are not `autoApply` because the `Double`-backed ones are, and two auto-applied
converters for the same type are ambiguous. An explicit `@Convert` on the field takes precedence over an
auto-applied converter, so opting in per field works with the built-ins still on the classpath.

### Tests

Unit-level only, per the decision to skip the Hibernate metadata IT. The round-trip assertions use `equals`,
not `compareTo`, and one test documents the defect from the other side — it asserts that the `Double`-backed
converter *does* lose `1999.50`'s scale and *does* mangle `12345678901234567.89`, so a future change that
quietly fixed or worsened the old path would be caught too.

The third test from the original plan — pinning the generated column type through Hibernate's metadata — was
deliberately not written. It needs Docker and a `@SpringBootTest`, and the mapping it would pin is already
determined by the converter's declared type parameter, which the unit tests cover. Worth revisiting if the
numeric converters ever become the auto-applied default.

### Documentation

`BaseBigDecimalTypeAttributeConverter`'s javadoc no longer claims a `Long` mapping; it names `Double` /
`double precision`, spells out both consequences (lost scale, floating-point SQL arithmetic), and points at
the numeric base class. `AmountAttributeConverter` and `PercentageAttributeConverter` gained short javadoc
carrying the same warning. `LLM/LLM-types-springdata-jpa.md`, `types-springdata-jpa/README.md` and
`types-springdata-jpa/CLAUDE.md` all list SQL types per converter now, and `docs/MIGRATION-NEXT_MAJOR.md`
carries the `alter column … type numeric(19,2)` note, including the point that the cast preserves what is in
the column and does not repair a figure float accumulation already corrupted.
