...
- **Coverage is decided by classpath, not by code.** Three modules extend the arch test, because no single classpath sees everything: `eventsourced-aggregates` (core chain + PostgreSQL event store), `spring-boot-starter-postgresql` (PostgreSQL queue + fenced lock), `spring-boot-starter-mongodb` (MongoDB queue + fenced lock). Where they overlap a violation lands in more than one store — harmless, `allowStoreUpdate` clears all of them when it is fixed.
