...
- **A dispatcher that cannot offer a `UnitOfWork`-free window rejects `NONE` rather than ignoring it** — `Inbox` (consumer doesn't own the boundary and a `UnitOfWorkFactory` is present, or `TransactionalMode.FullyTransactional`), `Outbox` (always, it has no boundary-owning consumer variant), `PatternMatchingQueuedMessageHandler` (always, at construction). All four skip the check when no `UnitOfWorkFactory` exists, because nothing wraps the delivery then and the handler does get its window. Guards pinned by `MessageHandlerMethodsTest`, `InboxNonTransactionalMessageHandlerGuardTest`, `OutboxNonTransactionalMessageHandlerGuardTest`, `PatternMatchingQueuedMessageHandlerTest`
