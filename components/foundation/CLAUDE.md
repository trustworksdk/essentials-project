...
- **The consumer's built-in permanent-error list beats the `RedeliveryPolicy`.** `DefaultDurableQueueConsumer.isPermanentError` and `CentralizedMessageFetcher.isPermanentError` carry an identical copy of the rule: the policy's `MessageDeliveryErrorHandler` is consulted first, then the built-in list is OR-ed on top, so no policy can remove a type from it. The list is `DurableQueueDeserializationException`, `MismatchedInputException` (root cause only), `NoClassDefFoundError`, `ClassCastException` and `IllegalArgumentException`. `IllegalArgumentException` on that list means `FailFast.requireNonNull`/`requireTrue` and Kotlin `require(...)` — the house validation idiom, 2000+ call sites — dead-letter a message on its first delivery when thrown inside a `@MessageHandler`, and `MessageDeliveryErrorHandler.builder().alwaysRetryOn(IllegalArgumentException.class)` does not change it. Two copies of the rule means a change to classification must edit both
- **Only the ends of the cause chain are classified, never the middle.** `isPermanentError` tests the thrown exception and `Exceptions.getRootCause(e)`, nothing between. A `@MessageHandler` throw arrives wrapped (`UnitOfWorkException → ReflectionException → InvocationTargetException → yours`), so the handler's own exception is normally deepest and decides — unless it carries a cause of its own, at which point classification silently switches to that deeper type. Two handlers differing only in whether they attach a cause get different dead-letter behaviour
