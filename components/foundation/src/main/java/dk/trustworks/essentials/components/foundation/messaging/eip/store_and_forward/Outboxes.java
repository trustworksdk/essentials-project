 ...
 * <ul>
 *   <li><b>Required Configuration:</b> The {@link Outbox} consumer MUST be configured with {@link OutboxConfig#getMessageConsumptionMode()} set to
 ...
 *       in the cluster actively consumes from the {@link Outbox} at any time, with automatic failover to standby nodes</li>
 *   <li><b>Parallel Processing:</b> The active node can still utilize multiple parallel threads (configured via
 *       {@link OutboxConfig#numberOfParallelMessageConsumers}) to process messages with different keys concurrently</li>
 ...
 * <pre>{@code
 * Outbox outbox = outboxes.getOrCreateOutbox(
 *     OutboxConfig.builder()
 *         .setOutboxName(OutboxName.of("NotificationsOutbox"))
