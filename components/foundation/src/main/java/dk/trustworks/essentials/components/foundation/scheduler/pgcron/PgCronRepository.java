 ...
 * <b>It is the responsibility of the user of this component to sanitize the {PgCronJob#cronExpression} and {PgCronJob#function}
 ...
 * validate {PgCronJob#cronExpression} and {PgCronJob#function} as an initial layer of defense against SQL injection by applying naming conventions intended to reduce the risk of malicious input.<br>
 ...
 * <br>
 * It is highly recommended that the {@code PgCronJob#cronExpression} and {@code PgCronJob#function} value is only derived from a controlled and trusted source.<br>
 * To mitigate the risk of SQL injection attacks, external or untrusted inputs should never directly provide the {@code PgCronJob#cronExpression} and {@code PgCronJob#function} values.<br>
 */
public class PgCronRepository {
     ...
     * the provided {@link PgCronJob}, which includes the cron expression and function name.
     * This method validates the function name, creates the necessary SQL query, and inserts
     * the scheduled job into the database. The ID of the scheduled job is then returned.
     * <p>
 *     See class security note. To mitigate the risk of SQL injection attacks, external or untrusted inputs should never directly provide the {@code PgCronJob#cronExpression} and {@code PgCronJob#function} values
     *
     * @param job the {@link PgCronJob} representing the cron job to be scheduled, which includes
     *            the function name and cron expression; must not be null
     * @return the ID of the scheduled job as an {@code Integer}, or {@code null} if the scheduling fails
     * @throws IllegalArgumentException if the job contains invalid data, such as an illegal function name
     */
    public Integer schedule(PgCronJob job) {
        requireNonNull(job, "job cannot be null");
        log.debug("Schedule PgCronJob '{}'", job);
        var cronExpression = job.cronExpression().toString();
        var functionName   = job.function();
        var jobName        = JobNameResolver.resolve(job.name());

        if (!FN_NAME.matcher(functionName).matches()) {
            throw new IllegalArgumentException(msg("Function name contains illegal characters '{}' ", functionName));
        }

        var innerQuery = "SELECT " + functionName + "();";

        var jobExists = doesJobExist(jobName);
        if (jobExists != null) {
            return jobExists;
        }

        var sql = "SELECT cron.schedule(:name, :cron, :query)::int;";
