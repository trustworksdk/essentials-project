 ...
 * <b>It is the responsibility of the user of this component to sanitize the {@code PgCronJob#cronExpression}, {@code PgCronJob#functionName} and  {@code PgCronJob#sqlCommand}
 ...
 * validate {@code PgCronJob#cronExpression}, {@code PgCronJob#functionName} and  {@code PgCronJob#sqlCommand} as an initial layer of defense against SQL injection by applying naming conventions intended to reduce the risk of malicious input.<br>
 ...
 * <br>
 * It is highly recommended that the {@code PgCronJob#cronExpression}, {@code PgCronJob#functionName} and  {@code PgCronJob#sqlCommand} value is only derived from a controlled and trusted source.<br>
 * To mitigate the risk of SQL injection attacks, external or untrusted inputs should never directly provide the {@code PgCronJob#cronExpression}, {@code PgCronJob#functionName} and  {@code PgCronJob#sqlCommand} values.<br>
 */
public class PgCronRepository {
    ...
    public Integer schedule(PgCronJob job) {
        ...
        var functionName   = job.functionName();
        var commandArgs    = job.sqlCommand();
        ...
        if (hasArgs) {
            var lower = commandArgs.trim().toLowerCase(Locale.ROOT);
            if (lower.startsWith("select") || commandArgs.contains(";")) {
                throw new IllegalArgumentException(
                        msg("SQL arguments must not start with SELECT or contain semicolons: '{}'", commandArgs)
                );
            }
        }

        var innerQuery = hasArgs
                            ? String.format("SELECT %s(%s);", functionName, commandArgs)
                            : String.format("SELECT %s();", functionName);

        var jobExists = doesJobExist(jobName);
        if (jobExists != null) {
            log.debug("Cron job '{}' already exists with id {}", jobName, jobExists);
            return jobExists;
        }
