 ...
 * validate {@link  PgCronJob#cronExpression()}, {@link  PgCronJob#functionName()} and  {@link  PgCronJob#args()} as an initial layer of defense against SQL injection by applying naming conventions intended to reduce the risk of malicious input.<br>
...
public class PgCronRepository {
    ...
    public Integer schedule(PgCronJob job) {
        ...
        boolean hasArgs = job.args() != null && !job.args().isEmpty();

        String innerQuery = hasArgs
                            ? String.format("SELECT %s(%s);", functionName, job.argsToCommaSepratedString())
                            : String.format("SELECT %s();", functionName);
        log.debug("Executing pg_cron query: '{}'", innerQuery);
