...
class MultiTableChangeListenerIT {
    ...
    void setup() {
        ...
        // The container is shared by every test in this class, so the tables survive between test methods - drop them first.
        jdbi.useTransaction(handle -> handle.execute("DROP TABLE IF EXISTS table1"));
        jdbi.useTransaction(handle -> handle.execute("DROP TABLE IF EXISTS table2"));
        jdbi.useTransaction(handle -> handle.execute("DROP TABLE IF EXISTS table3"));
