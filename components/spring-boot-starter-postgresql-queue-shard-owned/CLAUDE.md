...
- **The whole starter is off by default: `essentials.shard-owned-queue.enabled` defaults to `false`.** This is the reverse of the rest of Essentials, where a starter on the classpath counts as a request for what it configures. It is deliberate for the engine's first release (0.60). An enabled engine holds `pumpThreads + 1` pool connections for as long as the process runs and creates its own tables, so an application must ask for it explicitly. Both auto-configurations are `@ConditionalOnProperty(name = "enabled", havingValue = "true")` with no `matchIfMissing`, and `durable-queues-enabled` has no effect without it. `the_engine_stays_off_unless_enabled` and `durable_queues_enabled_alone_does_not_start_the_engine` guard this. The test runners set `enabled=true` explicitly.
