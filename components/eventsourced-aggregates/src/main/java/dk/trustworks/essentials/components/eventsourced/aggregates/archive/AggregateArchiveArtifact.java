...
public record AggregateArchiveArtifact(
    ...
    public AggregateArchiveArtifact {
        requireNonNull(format, "No format provided");
        requireNonNull(content, "No content provided");
        if (eventCount < 0) throw new IllegalArgumentException("eventCount must be >= 0");
        requireNonBlank(checksum, "No checksum provided");
