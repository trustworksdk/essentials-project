...
public class TransactionalBehaviorOnAggregateWithStateIT {
    ...
    private StatefulAggregateRepository<OrderId, Event<OrderId>, TestOrderWithState>    ordersRepository;
    ...
    @Test
    void concurrent_chained_events_on_new_aggregate_with_state_test() {
        log.info("---------- Scenario 1: Concurrent chained Event Handlers on a new Aggregate -------");
        ...
        // This triggers P1 to react to OrderCreated which triggers P2 to react to ProductAdded
    ...
    @Test
    void concurrent_chained_events_on_existing_aggregate_with_state_test() {
        log.info("---------- Scenario 1: Concurrent chained Event Handlers on an existing Aggregate -------");
        ...
        // This triggers P2 to react to OrderCreated and append another event to the same aggregate
        // Then: Should NOT throw exception
    ...
    // ========================================================================================================
    // SCENARIO 2: CONCURRENT CHAINED EVENT PROCESSORS
    // ========================================================================================================
    ...
    private static class OrderCommandHandler extends InTransactionEventProcessor {
        private final StatefulAggregateRepository<OrderId, Event<OrderId>, TestOrderWithState> repository;

        protected OrderCommandHandler(EventProcessorDependencies eventProcessorDependencies,
                                      StatefulAggregateRepository<OrderId, Event<OrderId>, TestOrderWithState> repository) {
    ...
    private static class EventProcessorP2 extends InTransactionEventProcessor {
        ...
        void on(TestOrderEvent.ProductAdded event) {
            log.debug("P2: Reacting to OrderCreated {} -> adding PRODUCT_B", event.aggregateId());
            repository.load(event.aggregateId()).appendProductData(event.productId, "productName");
        }
    }


    public static class TestOrderWithState extends AggregateRootWithState<OrderId, Event<OrderId>, TestOrderState, TestOrderWithState> {
        ...
        public void addProduct(ProductId productId) {
            ...
            apply(new TestOrderEvent.ProductAdded(productId));
        }

        public void appendProductData(ProductId productId, String productName) {
            requireNonNull(productId, "You must provide a productId");
            apply(new TestOrderEvent.ProductDataAppended(productId, productName));
        }

    }

    public static class TestOrderState extends AggregateState<OrderId, Event<OrderId>> {
    }
    ...
    public static class TestOrderEvent extends Event<OrderId> {
        ...
        public static class ProductAdded extends Event<OrderId> {
        ...
        public static class ProductDataAppended extends Event<OrderId> {
            public final ProductId productId;
            public final String productName;

            public ProductDataAppended(ProductId productId, String productName) {
                this.productId = productId;
