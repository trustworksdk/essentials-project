...
| `ClosingBooksSetup` / `ClosingBooksSetupBuilder` | Assembles generation repository + coordinator + derived `generationAccess()` for one aggregate type. `logicalAggregateRepository(delegate)` stays a caller-supplied step because only the app has the delegate. The setup is an `EssentialsSchemaContributor` for its repository's table (the setup is the bean, the repository is not); `setSchemaOwnership(...)` must be passed by the app outside `essentials.schema.mode=create` |
