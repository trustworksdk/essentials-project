...
- `TestObjectMappers.createJSONSerializer()` — the only way ITs build a serializer. Mapper config from `EssentialsObjectMappers.createJackson3ObjectMapper(...)` (that config *is* the persisted-JSON contract) plus Jackson 3's Kotlin module (`tools.jackson.module`), which is what binds the data classes' immutable constructors
...
- **Main code names no Jackson type** — it takes the `JSONSerializer` SPI. Only the tests need Jackson's Kotlin module (`tools.jackson.module`, `optional`). Keep it that way: serializer choice stays with the consumer.
