...
- **Every queue operation runs in its own transaction.** `SingleOperationTransactionDurableQueuesInterceptor` is wired unconditionally. The `TransactionalMode` enum was removed in 0.60: `FullyTransactional` was broken for retries and dead-lettering — rollback reverted the attempt count, so the redelivery policy never advanced — and with one constant left the type expressed a choice that no longer existed.
...
- **Centralized fetcher is default** (`useCentralizedMessageFetcher=true`).
