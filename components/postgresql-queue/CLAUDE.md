...
- **`useOrderedUnorderedQuery` flag**: when `true`, enables separate ordered/unordered CTE paths + corresponding partial indexes. Off by default. Both paths must stay in sync in `DurableQueuesSql`.
- **Centralized fetcher is default**: builder's `transactionalMode` defaults to `SingleOperationTransaction` (not `FullyTransactional`). `PostgresqlDurableQueues` multi-arg constructors default to `FullyTransactional` — builder vs constructor defaults differ.
