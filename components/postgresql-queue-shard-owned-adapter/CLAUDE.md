...
- **An unknown queue name fails by default, and the failure is now visible.** `DurableQueues` invents a queue on first use; this engine will not invent a shard count. `setAutoRegisterShardCount(n)` opts in, and commits every new queue to a number nobody chose. The throw comes from `resolve`, which for an `Inbox` in `SingleGlobalConsumer` mode lands inside the fenced lock's `onLockAcquired` callback — that used to leave the lock held with the inbox silently never consuming. `DBFencedLockManager` now releases the lock when a callback throws, so the message is logged each tick and the inbox starts on its own once the queue is registered, with no restart. See `foundation/CLAUDE.md`.
