 ...
 */
@Testcontainers
abstract class SingleOperationTransactionMsSqlDistributedCompetingConsumersDurableQueuesIT extends DistributedCompetingConsumersDurableQueuesIT<MsSqlDurableQueues, GenericHandleAwareUnitOfWorkFactory.GenericHandleAwareUnitOfWork, JdbiUnitOfWorkFactory> {
    @Container
    protected final MSSQLServerContainer<?> msSQLContainer = dk.trustworks.essentials.components.queue.mssql.MsSqlTestContainerSupport.createMsSqlContainer();
    ...
    protected void disruptDatabaseConnection() {
        var dockerClient = msSQLContainer.getDockerClient();
        dockerClient.pauseContainerCmd(msSQLContainer.getContainerId()).exec();
    }

    @Override
    protected void restoreDatabaseConnection() {
        var dockerClient = msSQLContainer.getDockerClient();
        dockerClient.unpauseContainerCmd(msSQLContainer.getContainerId()).exec();
    }
    ...
    protected JdbiUnitOfWorkFactory createUnitOfWorkFactory() {
        ...
        ds.setJdbcUrl(msSQLContainer.getJdbcUrl());
        ds.setUsername(msSQLContainer.getUsername());
        ds.setPassword(msSQLContainer.getPassword());
