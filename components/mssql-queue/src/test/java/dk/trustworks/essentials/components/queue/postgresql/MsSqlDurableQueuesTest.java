...
class MsSqlDurableQueuesTest {

    @Test
    void initializeWithDefaultTableName() {
        var durableQueues = new MsSqlDurableQueues(
                mock(HandleAwareUnitOfWorkFactory.class),
                mock(JSONSerializer.class),
                MsSqlDurableQueues.DEFAULT_DURABLE_QUEUES_TABLE_NAME,
                mock(MultiTableChangeListener.class),
                null,
                mock(TransactionalMode.class),
                mock(Duration.class));
        assertThat(durableQueues.getSharedQueueTableName()).isEqualTo(MsSqlDurableQueues.DEFAULT_DURABLE_QUEUES_TABLE_NAME);
    }
