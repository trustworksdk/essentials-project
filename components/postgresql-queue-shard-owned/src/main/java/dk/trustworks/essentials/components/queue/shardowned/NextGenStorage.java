...
import static dk.trustworks.essentials.components.queue.shardowned.NextGenSchema.*;
 ...
 */
public final class NextGenStorage {
    private final DataSource dataSource;
    private final short      queueId;

    public NextGenStorage(DataSource dataSource, short queueId) {
    ...
    public void enqueueBatch(Connection connection, int shard, List<byte[]> payloads, int payloadType) throws SQLException {
        inTransaction(connection, () -> {
            enqueueBatchInternal(connection, shard, payloads, payloadType);
            return null;
        });
    }

    private void enqueueBatchInternal(Connection connection, int shard, List<byte[]> payloads, int payloadType) throws SQLException {
        var sql = "INSERT INTO " + UNORDERED_TABLE
                  + " (queue_id, shard, seq, payload, payload_type) VALUES (?, ?, nextval(?::regclass), ?, ?)";
        try (var statement = connection.prepareStatement(sql)) {
            for (var payload : payloads) {
                statement.setShort(1, queueId);
                statement.setShort(2, (short) shard);
                statement.setString(3, NextGenSchema.sequenceName(queueId, shard));
        ...
        NextGenListener.notifyShard(connection, queueId, "unordered", shard);
    }
    ...
    public List<Row> readFromCursor(Connection connection, int shard, long cursor, int limit, long ownFence) throws SQLException {
        ...
        var sql = "SELECT seq, payload FROM " + UNORDERED_TABLE
    ...
    /**
     * Fencing (§4.7): a write only lands if the writer still holds the lease it was issued under.
     * <p>
    ...
    public long nextSessionFence() throws SQLException {
        try (var connection = dataSource.getConnection();
             var statement = connection.prepareStatement("SELECT nextval('" + NextGenSchema.SESSION_FENCE_SEQUENCE + "')");
    ...
    private static List<Row> readRows(PreparedStatement statement) throws SQLException {
        try (var resultSet = statement.executeQuery()) {
            ...
            while (resultSet.next()) {
                rows.add(new Row(resultSet.getLong(1), resultSet.getBytes(2)));
            }
            return rows;
        }
    }
     ...
     */
    public void enqueueOrderedBatch(Connection connection, int shard, List<OrderedPayload> messages, int payloadType) throws SQLException {
        inTransaction(connection, () -> {
            enqueueOrderedBatchInternal(connection, shard, messages, payloadType);
            return null;
        });
    }

    private void enqueueOrderedBatchInternal(Connection connection, int shard, List<OrderedPayload> messages, int payloadType) throws SQLException {
        var sql = "INSERT INTO " + ORDERED_TABLE
                  + " (queue_id, shard, msg_key, key_order, seq, payload, payload_type)"
                  + " VALUES (?, ?, ?, ?, nextval(?::regclass), ?, ?)";
        try (var statement = connection.prepareStatement(sql)) {
            for (var message : messages) {
                ...
                statement.setString(5, NextGenSchema.orderedSequenceName(queueId, shard));
                statement.setBytes(6, message.payload());
                statement.setInt(7, payloadType);
                statement.addBatch();
            }
            statement.executeBatch();
        }
        NextGenListener.notifyShard(connection, queueId, "ordered", shard);
    }
    ...
    public List<OrderedRow> readOrderedFromCursor(Connection connection, int shard, long cursor, int limit) throws SQLException {
        var sql = "SELECT seq, msg_key, key_order, payload FROM " + ORDERED_TABLE
                  + " WHERE queue_id = ? AND shard = ? AND seq > ? AND visible_at <= now()"
                  + " ORDER BY seq LIMIT " + limit;
        try (var statement = connection.prepareStatement(sql)) {
            statement.setShort(1, queueId);
    ...
    public List<DeadLetter> deadLetters() throws SQLException {
        try (var connection = dataSource.getConnection();
             var statement = connection.prepareStatement(
                     "SELECT source_lane, msg_key, seq, payload, attempts, last_error, shard FROM " + DLQ_TABLE
    ...
    public boolean resurrect(int shard, long seq, String lane) throws SQLException {
        var table = "ordered".equals(lane) ? ORDERED_TABLE : UNORDERED_TABLE;
        var columns = "ordered".equals(lane)
                      ? "queue_id, shard, msg_key, key_order, seq, payload, payload_type, attempts"
                      : "queue_id, shard, seq, payload, payload_type, attempts";
        var selected = "ordered".equals(lane)
                       ? "queue_id, shard, msg_key, key_order, seq, payload, payload_type, 0"
                       : "queue_id, shard, seq, payload, payload_type, 0";
        try (var connection = dataSource.getConnection()) {
            connection.setAutoCommit(false);
            try {
                int restored;
                ...
                NextGenListener.notifyShard(connection, queueId, lane, shard);
                connection.commit();
                return true;
            } catch (SQLException e) {
                connection.rollback();
    ...
    public record OrderedPayload(String key, long keyOrder, byte[] payload) {
    }

    public record OrderedRow(long seq, String key, long keyOrder, byte[] payload) {
    }

    public record Row(long seq, byte[] payload) {
    }
}
