 ...
 */
public final class NextGenQueue implements Lifecycle, AutoCloseable {
    private static final Logger log = LoggerFactory.getLogger(NextGenQueue.class);
    ...
    private Consumer<byte[]>                activeHandler;
    private java.util.function.BiConsumer<String, byte[]> activeOrderedHandler;
    ...
    public NextGenQueue setParallelConsumers(int parallelConsumers) {
    ...
    public NextGenQueue(DataSource dataSource, short queueId, int shardCount, String instanceId) {
    ...
    public void enqueue(List<byte[]> payloads, int payloadType) throws SQLException {
        ...
        for (var index = 0; index < seqs.size(); index++) {
            owner.handOffLocally(new NextGenStorage.Row(seqs.get(index), payloads.get(index)));
        }
    }

    /**
     * Lease every shard this instance can take and start owning them.
     *
     ...
     */
    public void enqueueOrdered(List<NextGenStorage.OrderedPayload> messages, int payloadType) throws SQLException {
        ...
        try (var connection = dataSource.getConnection()) {
            for (var entry : byShard.entrySet()) {
                storage.enqueueOrderedBatch(connection, entry.getKey(), entry.getValue(), payloadType);
            }
        }
    }
     ...
     */
    public void startConsumingOrdered(BiConsumer<String, byte[]> handler, ShardOwnerSettings settings, int maxShards) throws SQLException {
        startConsumingOrdered(handler, settings, maxShards, RedeliveryPolicy.fixed(java.time.Duration.ofMillis(50), 3));
    }

    public void startConsumingOrdered(BiConsumer<String, byte[]> handler, ShardOwnerSettings settings, int maxShards, RedeliveryPolicy redeliveryPolicy) throws SQLException {
     ...
     * start time.
     */
    public NextGenQueue configureOrdered(BiConsumer<String, byte[]> handler, ShardOwnerSettings settings,
    ...
    public NextGenQueue configureUnordered(Consumer<byte[]> handler, ShardOwnerSettings settings,
                                           int maxShards, RedeliveryPolicy redeliveryPolicy) {
    ...
    private void startOrdered() throws SQLException {
        ...
        leaseTtlMillis = Math.max(1_000L, settings.holeExpiry().toMillis() * 3);
    ...
    public void startConsuming(Consumer<byte[]> handler, ShardOwnerSettings settings, int maxShards) throws SQLException {
        startConsuming(handler, settings, maxShards, RedeliveryPolicy.fixed(java.time.Duration.ofMillis(50), 3));
    }

    public void startConsuming(Consumer<byte[]> handler, ShardOwnerSettings settings, int maxShards, RedeliveryPolicy redeliveryPolicy) throws SQLException {
    ...
    public void stop() {
        ...
        // other queues; one this queue created is closed with it.
        releaseRuntime();
        owners.clear();
        ownedShards.clear();
    }
    ...
    public List<NextGenStorage.DeadLetter> deadLetters() throws SQLException {
