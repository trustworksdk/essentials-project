...
class NextGenQueueIT {
                ...
                futures.add(executor.submit(() -> {
                    for (var index = 0; index < perProducer; index++) {
                        try (var connection = dataSource.getConnection()) {
                            ...
                            new NextGenStorage(dataSource, QUEUE_ID)
    ...
    void a_new_owner_redelivers_what_a_crashed_owner_left_unacknowledged() throws Exception {
        ...
        var first = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "instance-1");
        ...
        var shortLease = new ShardOwnerSettings(500, 200,
                                                ...
                                                1_000, 200L, 8, Duration.ofMillis(100), Duration.ofSeconds(30), 512, 2, Duration.ofSeconds(5));
        first.startConsuming(payload -> {
        ...
        try (var second = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "instance-2")) {
            second.startConsuming(payload -> secondReceived.add(new String(payload, StandardCharsets.UTF_8)),
                                  ShardOwnerSettings.defaults(),
                                  SHARD_COUNT);
    ...
    void two_instances_split_the_shards_and_do_not_deliver_the_same_message_twice() throws Exception {
        ...
        try (var first = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "instance-1");
             var second = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "instance-2")) {

            first.startConsuming(payload -> firstReceived.add(new String(payload, StandardCharsets.UTF_8)),
                                 ShardOwnerSettings.defaults(),
                                 SHARD_COUNT / 2);
            second.startConsuming(payload -> secondReceived.add(new String(payload, StandardCharsets.UTF_8)),
                                  ShardOwnerSettings.defaults(),
