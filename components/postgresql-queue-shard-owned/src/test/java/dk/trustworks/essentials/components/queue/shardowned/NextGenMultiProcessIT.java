...
import dk.trustworks.essentials.components.queue.shardowned.node.NextGenNodeMain;
...
class NextGenMultiProcessIT {
    private static final Logger log = LoggerFactory.getLogger(NextGenMultiProcessIT.class);
    ...
    void setUp() throws Exception {
        ...
        try (var connection = dataSource.getConnection();
            ...
            statement.execute("DROP TABLE IF EXISTS ng_observed");
    ...
    void ordering_per_key_holds_across_processes_and_lease_lifetimes() throws Exception {
        ...
        var keyCount = 40;
        var perKey = 20;
        var queue = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "producer");
        ...
        for (var order = 0; order < perKey; order++) {
            ...
            for (var key = 0; key < keyCount; key++) {
                batch.add(new NextGenStorage.OrderedPayload("k-" + key, order, encode(order)));
            }
            queue.enqueueOrdered(batch, 1);
            Thread.sleep(750L);
    ...
    private long observedCount() throws SQLException {
        try (var connection = dataSource.getConnection();
             var statement = connection.createStatement();
             var resultSet = statement.executeQuery("SELECT count(*) FROM ng_observed")) {
            resultSet.next();
    ...
    private Set<Long> distinctObservedValues() throws SQLException {
        try (var connection = dataSource.getConnection();
             var statement = connection.createStatement();
             var resultSet = statement.executeQuery("SELECT DISTINCT value FROM ng_observed")) {
    ...
    private Set<String> liveLeaseOwners() throws SQLException {
        try (var connection = dataSource.getConnection();
             var statement = connection.prepareStatement(
                     "SELECT DISTINCT owner FROM " + NextGenSchema.LEASE_TABLE
    ...
    private Map<String, Long> observationsPerInstance() throws SQLException {
        try (var connection = dataSource.getConnection();
             ...
             var resultSet = statement.executeQuery(
                     "SELECT instance_id, count(*) FROM ng_observed GROUP BY instance_id")) {
    ...
    private Map<String, Long> deliveriesAfterFirstDeliveryBy(String instanceId) throws SQLException {
        var sql = """
                  SELECT instance_id, count(*) FROM ng_observed
                  WHERE id > (SELECT min(id) FROM ng_observed WHERE instance_id = ?)
    ...
    private List<String> orderingViolations() throws SQLException {
        var sql = """
                  SELECT msg_key FROM (
                      ...
                      FROM ng_observed WHERE msg_key IS NOT NULL
                  ) ordered
