...
class NextGenPullSessionIT {
    ...
    void a_session_excludes_consumers_while_it_holds_its_shards_and_releases_them_on_close() throws Exception {
        try (var queue = queue("pull-2")) {
            ...
            try (var other = queue("consumer")) {
                var subscription = other.consume((key, payload) -> {
    ...
    /**
     * Every scope now resolves to a session, except the one that cannot exist in this engine. MESSAGE
     * and BATCH take individual rows and are covered by {@code NextGenRowLeaseSessionIT}; KEY is
     * refused because per-key exclusivity is enforced in the owner's memory and a second party cannot
     * enter it — see {@link SessionScope#KEY}.
     */
    @Test
    void every_scope_opens_a_session_except_the_one_that_cannot_exist_here() throws Exception {
        try (var queue = queue("pull-4")) {
            for (var scope : new SessionScope[]{SessionScope.SHARD, SessionScope.MESSAGE, SessionScope.BATCH}) {
