...
class NextGenDatabaseFailureIT {
    private static final Logger log = LoggerFactory.getLogger(NextGenDatabaseFailureIT.class);
    ...
    void the_engine_recovers_when_the_connection_pool_is_starved() throws Exception {
        ...
        try (var small = pool(8);
             var queue = new NextGenQueue(small, QUEUE_ID, SHARD_COUNT, "instance-1")) {
            queue.startConsuming(payload -> received.add(new String(payload, StandardCharsets.UTF_8)),
    ...
    void a_permanent_write_failure_does_not_lose_data_and_recovers_when_repaired() throws Exception {
        ...
        try (var queue = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "instance-1")) {
            ...
            assertThatThrownBy(() -> execute("DELETE FROM " + NextGenSchema.UNORDERED_TABLE + " WHERE false"))
    ...
    void a_batch_enqueue_is_all_or_nothing() throws Exception {
        try (var queue = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "instance-1")) {
            ...
            execute("CREATE OR REPLACE FUNCTION ng_reject_poison() RETURNS trigger AS $$ "
                    + "BEGIN IF encode(NEW.payload, 'escape') LIKE '%POISON%' "
                    + "THEN RAISE EXCEPTION 'rejected'; END IF; RETURN NEW; END; $$ LANGUAGE plpgsql");
            execute("CREATE TRIGGER ng_reject_poison BEFORE INSERT ON " + NextGenSchema.UNORDERED_TABLE
                    + " FOR EACH ROW EXECUTE FUNCTION ng_reject_poison()");
            ...
            execute("DROP TRIGGER IF EXISTS ng_reject_poison ON " + NextGenSchema.UNORDERED_TABLE);
        }
    }
    ...
    private void breakAcknowledgement() throws Exception {
        execute("CREATE OR REPLACE FUNCTION ng_block_delete() RETURNS trigger AS $$ "
                + "BEGIN RAISE EXCEPTION 'acknowledgement disabled'; END; $$ LANGUAGE plpgsql");
        execute("CREATE TRIGGER ng_block_delete BEFORE DELETE ON " + NextGenSchema.UNORDERED_TABLE
                + " FOR EACH STATEMENT EXECUTE FUNCTION ng_block_delete()");
    }

    private void repairAcknowledgement() throws Exception {
        execute("DROP TRIGGER IF EXISTS ng_block_delete ON " + NextGenSchema.UNORDERED_TABLE);
    }
