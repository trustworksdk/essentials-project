...
class NextGenOrderedRebalanceIT {
    ...
    private static ShardOwnerSettings fast(Duration shedGrace) {
        return new ShardOwnerSettings(500, 200, Duration.ofMillis(1), Duration.ofMillis(2),
                                      Duration.ofMillis(300), Duration.ofMillis(100), 1_000, 200L, 8,
                                      Duration.ofMillis(50), Duration.ofSeconds(30), 512, 2, shedGrace);
    }
    ...
    private void enqueueOrdered(NextGenQueue queue, int keys, int perKey) throws Exception {
        var messages = new ArrayList<NextGenStorage.OrderedPayload>();
        for (var order = 0; order < perKey; order++) {
            for (var key = 0; key < keys; key++) {
                messages.add(new NextGenStorage.OrderedPayload("key-" + key, order,
                                                               ("key-" + key + "#" + order).getBytes(StandardCharsets.UTF_8)));
            }
        }
     ...
     */
    @Test
    void a_joining_instance_gets_its_fair_share_because_the_incumbent_drains_and_releases() throws Exception {
        var first  = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "ord-a");
        var second = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "ord-b");
        try {
            first.startConsumingOrdered((key, payload) -> {
            }, fast(Duration.ofSeconds(5)), SHARD_COUNT);
            Awaitility.await().atMost(Duration.ofSeconds(10))
                      .untilAsserted(() -> assertThat(first.shardsHeld()).isEqualTo(SHARD_COUNT));

            second.startConsumingOrdered((key, payload) -> {
            }, fast(Duration.ofSeconds(5)), SHARD_COUNT);
    ...
    void no_key_is_ever_in_two_handlers_at_once_while_shards_move() throws Exception {
        ...
        java.util.function.BiConsumer<String, byte[]> handler = (key, payload) -> {
        ...
        var first  = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "ord-c");
        var second = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "ord-d");
        try {
    ...
    void a_shed_that_cannot_drain_is_abandoned_and_the_shard_stays_where_it_is() throws Exception {
        ...
        java.util.function.BiConsumer<String, byte[]> stuck = (key, payload) -> {
            inHandler.countDown();
            try {
        ...
        var first  = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "ord-e");
        var second = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "ord-f");
        try {
