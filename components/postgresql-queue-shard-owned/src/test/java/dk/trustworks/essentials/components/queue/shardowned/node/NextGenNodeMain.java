 ...
 */
public final class NextGenNodeMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var queue = new NextGenQueue(dataSource, (short) 1, shardCount, instanceId);
        if ("ordered".equals(lane)) {
            queue.startConsumingOrdered((key, payload) -> record(dataSource, instanceId, key,
                                                                 ByteBuffer.wrap(payload).getLong()),
                                        settings, maxShards);
        } else {
            queue.startConsuming(payload -> record(dataSource, instanceId, null,
                                                   ByteBuffer.wrap(payload).getLong()),
                                 settings, maxShards);
        }
    ...
    private static void record(HikariDataSource dataSource, String instanceId, String key, long value) {
        try (var connection = dataSource.getConnection();
             var statement = connection.prepareStatement(
                     "INSERT INTO ng_observed (instance_id, msg_key, value) VALUES (?, ?, ?)")) {
