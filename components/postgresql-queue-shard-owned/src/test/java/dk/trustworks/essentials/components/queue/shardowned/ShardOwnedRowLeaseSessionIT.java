...
class ShardOwnedRowLeaseSessionIT {
    ...
    void two_sessions_pulling_at_once_never_receive_the_same_message() throws Exception {
        ...
        try (var queue = queue("row-2")) {
            ...
            // Keyed by the whole MessageId. Sequences are per (queue, shard), so seq 1 exists in
            // every shard and deduping on the sequence alone counts distinct messages as
            // duplicates the moment an enqueue spreads across more than one shard.
            var seen  = ConcurrentHashMap.<MessageId>newKeySet();
