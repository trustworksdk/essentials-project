...
class NextGenMultiQueueCostIT {
    private static final Logger log = LoggerFactory.getLogger(NextGenMultiQueueCostIT.class);
    ...
    void setUp() throws Exception {
        ...
        for (var queueId = 1; queueId <= QUEUES; queueId++) {
            NextGenSchema.registerQueue(dataSource, (short) queueId, SHARD_COUNT);
        }
    }
    ...
    private static ShardOwnerSettings settings() {
        ...
        return new ShardOwnerSettings(defaults.readBatchSize(), defaults.ackBatchSize(), defaults.ackFlushInterval(),
                                      ...
                                      defaults.maxHolesPerChase(), defaults.idleParkMicros(),
                                      defaults.keyConcurrency(), defaults.pollBackstop(), Duration.ofSeconds(30), 512, PUMP_THREADS,
                                      defaults.shedGrace());
    }
    ...
    void records_what_several_idle_queues_cost_in_threads_connections_and_queries() throws Exception {
        ...
        var queues = new ArrayList<NextGenQueue>();
        ...
        try {
            for (var queueId = 1; queueId <= QUEUES; queueId++) {
                var unordered = new NextGenQueue(dataSource, (short) queueId, SHARD_COUNT, "cost-u-" + queueId, runtime);
                unordered.startConsuming(payload -> {
                }, settings(), SHARD_COUNT);
                var ordered = new NextGenQueue(dataSource, (short) queueId, SHARD_COUNT, "cost-o-" + queueId, runtime);
                ordered.startConsumingOrdered((key, payload) -> {
                }, settings(), SHARD_COUNT);
            ...
            for (var index = 0; index < Math.min(queues.size(), 10); index += 2) {
                ...
                var batch = new ArrayList<NextGenStorage.OrderedPayload>();
                for (var key = 0; key < 64; key++) {
                    batch.add(new NextGenStorage.OrderedPayload("k-" + key, 0L,
                                                                ("x-" + key).getBytes(StandardCharsets.UTF_8)));
                }
                ordered.enqueueOrdered(batch, 1);
            }
            ...
            queues.forEach(NextGenQueue::stop);
            runtime.close();
        }
    }
    ...
    void several_queues_do_not_manufacture_holes_for_each_other() throws Exception {
        ...
        try {
            for (var queueId = 1; queueId <= QUEUES; queueId++) {
                ...
                var queue = new NextGenQueue(dataSource, (short) queueId, SHARD_COUNT, "seq-" + queueId, runtime);
                ...
                queue.startConsuming(payload -> delivered.get(id).add(new String(payload, StandardCharsets.UTF_8)),
                                     settings(), SHARD_COUNT);
                queues.add(queue);
            }
