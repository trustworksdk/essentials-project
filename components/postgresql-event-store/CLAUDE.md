...
- **`CdcMode.AUTO` vs `REQUIRE`**: `AUTO` silently falls back to polling if WAL logical replication is unavailable; `REQUIRE` fails startup. The `CdcEffectivenessMonitor` tracks whether CDC is actually delivering.
