...
Slot names are generated by [`CdcSlotNameProvider`](CdcSlotNameProvider.java).
...
Override the consumer-group via [`CdcProperties.slot.group`](CdcProperties.java) when
you want multiple independent CDC pipelines (e.g. one per bounded context) sharing the
same database.

### 3.2 Slot ownership & contention
...
### 3.3 Slot lifecycle modes (`PgSlotMode`)

Configured via [`CdcProperties.slot.mode`](CdcProperties.java):
...
### 3.4 Slot validation diagnostics
...
Three mechanisms in [`WalReplicationTailer`](WalReplicationTailer.java):
...
When [`CdcDispatcher`](CdcDispatcher.java) fails to decode a row, the
...
- `QUARANTINE_AND_CONTINUE` (default):
  ...
  4. Notify [`CdcPoisonNotifier`](CdcPoisonNotifier.java) — the default
     [`SubscriptionResetOnPoisonNotifier`](SubscriptionResetOnPoisonNotifier.java)
...
All keys live under `cdc.*` in [`CdcProperties`](CdcProperties.java).
...
### 7.2 `cdc.slot` ([`CdcSlotProperties`](CdcProperties.java))
...
### 7.4 `cdc.walReplicationTailer` ([`WalReplicationTailerProperties`](CdcProperties.java))
...
### 7.7 `cdc.healthCheck` ([`CdcHealthCheckProperties`](CdcProperties.java))
...
Setting different `cdc.slot.group` values (e.g. `orders`, `billing`, `default`)
produces independent slots, each with its own advisory lock. With multiple nodes,
this naturally distributes tailer load: each slot is grabbed by whichever node
acquires its lock first, and they spread across the cluster without explicit
coordination.

There is **no built-in load balancer** that tries to even out slots-per-node. In
practice the spread is good enough — locks are independent and acquisition is
race-driven — but a deliberate operator can pin slots to specific nodes via
...
| File                                                                                       | Purpose                                                                          |
| ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- |
| [`CdcEventStore.java`](CdcEventStore.java)                                                 | Decorates the EventStore; switches live source between CDC bus and polling.      |
| [`WalReplicationTailer.java`](WalReplicationTailer.java)                                   | Owns the replication stream; writes inbox or pushes to bus directly.             |
| [`CdcInboxRepository.java`](CdcInboxRepository.java) / [`CdcSql.java`](CdcSql.java)        | `eventstore_cdc_inbox` table DDL/queries.                                        |
| [`CdcDispatcher.java`](CdcDispatcher.java)                                                 | Polls the inbox, decodes, publishes to the bus.                                  |
| [`CdcEventBus.java`](CdcEventBus.java) / [`CdcSinkEmitter.java`](CdcSinkEmitter.java)       | Per-aggregate Reactor sinks + emit retry policy.                                 |
| [`CdcAvailability.java`](CdcAvailability.java)                                             | INACTIVE / ACTIVE / FAILED state machine.                                        |
| [`CdcEffectivenessMonitor.java`](CdcEffectivenessMonitor.java)                             | Background "is CDC actually delivering" probe.                                   |
| [`CdcPoisonNotifier.java`](CdcPoisonNotifier.java) / [`SubscriptionResetOnPoisonNotifier.java`](SubscriptionResetOnPoisonNotifier.java) / [`PoisonPolicy.java`](PoisonPolicy.java) | Poison-row handling. |
| [`PgReplicationSlots.java`](PgReplicationSlots.java) / [`PgSlotMode.java`](PgSlotMode.java) | Slot lifecycle + validation.                                                     |
| [`CdcSlotNameProvider.java`](CdcSlotNameProvider.java) / [`DefaultCdcSlotNameProvider.java`](DefaultCdcSlotNameProvider.java) | Slot naming.                                                |
| [`LogicalDecodingPlugin.java`](LogicalDecodingPlugin.java)                                 | Plugin abstraction.                                                              |
| [`PgOutputLogicalDecodingPlugin.java`](PgOutputLogicalDecodingPlugin.java) + [`PgOutputMessageDecoder.java`](PgOutputMessageDecoder.java) + [`PgOutputRowChangeDecoder.java`](PgOutputRowChangeDecoder.java) + [`PgOutputRowChange.java`](PgOutputRowChange.java) | pgoutput plugin + decoder. |
| [`Wal2JsonLogicalDecodingPlugin.java`](Wal2JsonLogicalDecodingPlugin.java)                 | wal2json plugin.                                                                 |
| [`CdcProperties.java`](CdcProperties.java) / [`CdcMode.java`](CdcMode.java) / [`CdcConsumerGroup.java`](CdcConsumerGroup.java) | Configuration.                                                  |
| [`CdcBusOverflowException.java`](CdcBusOverflowException.java) / [`StaleReplicationStreamException.java`](StaleReplicationStreamException.java) | Exception types.                                                |
