...
/**
 * Interface defining a contract for converting logical replication messages into persisted events.
 ...
 */
public interface LogicalReplicationToPersistedEventConverter {

    /**
     * Convert a single logical replication payload (which may include many changes)
     * into zero or more PersistedEvent.
     * <p>
     * Return empty list if the message is not relevant.
     */
    List<PersistedEvent> convert(String message);

    default List<PersistedEvent> convert(byte[] messageBytes) {
        if (messageBytes == null || messageBytes.length == 0) return List.of();
        return convert(new String(messageBytes, StandardCharsets.UTF_8));
    }
}
