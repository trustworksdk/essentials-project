...
- **New concrete domain type** → subclass one of the base classes (e.g. `extends LongType<MyId> implements Identifier`)
  ...
  - A `NumberType` subclass needs no extra constructor for JSON — `types-jackson3` deserializes the family at the wrapped width. Convenience overloads are not part of the wire contract; see `LLM/LLM-types-jackson.md` → *NumberType deserialization* for the trap this replaced
