...
- **Schema DDL lives in schema contributors** — a component describes its tables as `SchemaChange`s (`EssentialsSchemaContributor`) and builds with the `SchemaOwnership` the caller passes; in Spring every schema-owning bean takes `properties.getSchema().getMode().schemaOwnership()`, or it silently runs DDL in `essentials.schema.mode=validate`. Statements must be safe against an existing object (no adoption path). DDL elsewhere fails the unfrozen `EssentialsSchemaRules` guard. Design: `docs/database-schema-harness.md`
