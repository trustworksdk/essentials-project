...
- **Kotlin code needs `-java-parameters` for that contract to work at all.** Kotlin does not emit Java `MethodParameters` metadata unless the compiler is told to, so `Parameter#getName()` answers `arg0`, `arg1`, … and J3's properties-based creator binds every constructor parameter to `null` — commands, events and request bodies alike, in the web layer *and* when reading events back out of the store. It surfaces as Kotlin's own `Parameter specified as non-null is null: … parameter id`, which reads like a caller bug rather than a build-configuration one. `kotlin-maven-plugin` gets `<arg>-java-parameters</arg>`; `jackson-module-kotlin` is not needed for `data class`es. First hit in `examples/essentials-webshop-demo`, and it applies to any Kotlin consumer
