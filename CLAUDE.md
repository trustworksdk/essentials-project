...
- **A version property is not proof of a version** — an imported BOM below `spring-boot-dependencies` in the root `dependencyManagement` is silently inert (first import wins); a *direct* entry outranks every import wherever it sits. `jackson-bom`, `mockito-bom` and a dead `junit-bom.version` all read as if they applied while Spring Boot's versions resolved. Order rationale is commented at that block in `pom.xml`. Confirm every version claim with `mvn dependency:tree`, never by reading a POM
