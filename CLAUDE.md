...
- **`FailFast` inside a `@MessageHandler` dead-letters the message on first delivery** — the queue consumer OR-s a built-in permanent-error list *after* consulting the `RedeliveryPolicy`'s `MessageDeliveryErrorHandler`, and `IllegalArgumentException` is on it. `requireNonNull`/`requireTrue` and Kotlin `require(...)` all throw it, and `alwaysRetryOn(IllegalArgumentException.class)` does not override it. Throw a retryable exception when the condition may become true later; details in `LLM/LLM-foundation.md`
