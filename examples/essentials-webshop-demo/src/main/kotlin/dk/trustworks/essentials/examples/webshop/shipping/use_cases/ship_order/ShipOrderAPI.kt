...
@RestController
class ShipOrderAPI(private val commandBus: CommandBus) {

    data class ShipOrderRequest(val trackingNumber: TrackingNumber)

    @PostMapping("/api/shipping/orders/{orderId}/ship")
    fun shipOrder(
        @PathVariable orderId: OrderId,
        @RequestBody request: ShipOrderRequest
    ): ResponseEntity<Void> {
        val event: OrderShipped? = commandBus.send(ShipOrder(orderId, request.trackingNumber))
