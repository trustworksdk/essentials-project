 ...
 *
 * `shipping` and `payment` both subscribe to [OrderPlaced], and `shipping` also reads [ShippingDetailsAdded].
 * That makes these three the published contract of the `sales` context, and the reason its `events/` package is
 * importable from elsewhere while everything under `use_cases/` is not.
 */
