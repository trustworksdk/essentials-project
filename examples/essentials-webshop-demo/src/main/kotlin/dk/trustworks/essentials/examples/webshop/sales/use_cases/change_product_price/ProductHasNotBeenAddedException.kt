...
/**
 * The price of a product nobody has added cannot be changed.
 *
 * A rejection, so it throws. A command that has already been applied is a different case entirely: the decider
 * returns no event for that, and the caller sees success.
 *
 * The message includes the id because `GivenWhenThenScenario.thenFailsWithException` compares the exception's
