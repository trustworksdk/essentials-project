...
/**
 * The API file of the {@code brokerage.reserve_funds} slice (rules/slice-design.md §R2).
 *
 ...
 *
 * <p>Uses {@code send} rather than {@code sendAndDontWait} so a caller that reserves before placing a trade knows the
 * reservation held.
 */
@RestController
@RequestMapping(path = "/api/admin/trading-accounts")
public class ReserveFundsAPI {
    private final CommandBus commandBus;

    public ReserveFundsAPI(CommandBus commandBus) {
        this.commandBus = requireNonNull(commandBus, "No commandBus provided");
    }

    @PostMapping("/{accountId}/fund-reservations")
    public void reserveFunds(@PathVariable TradingAccountId accountId,
                             @RequestBody ReserveFunds cmd) {
        commandBus.send(reconcile(accountId, cmd));
    }

    private ReserveFunds reconcile(TradingAccountId accountId, ReserveFunds cmd) {
        if (cmd.accountId() == null) return new ReserveFunds(accountId, cmd.amount());
