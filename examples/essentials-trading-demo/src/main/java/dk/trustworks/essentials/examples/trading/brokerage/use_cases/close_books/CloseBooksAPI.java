...
/**
 * The API file of the {@code brokerage.close_books} slice (rules/slice-design.md §R2).
 *
 ...
 *
 * <p>Uses {@code send} rather than {@code sendAndDontWait} so an admin reading the generation view straight after this
 * call sees the sealed generation.
 */
@RestController
@RequestMapping(path = "/api/admin/trading-accounts")
public class CloseBooksAPI {
    private final CommandBus commandBus;

    public CloseBooksAPI(CommandBus commandBus) {
        this.commandBus = requireNonNull(commandBus, "No commandBus provided");
    }

    @PostMapping("/{accountId}/books-closures")
    public void closeBooks(@PathVariable TradingAccountId accountId,
                           @RequestBody CloseBooks cmd) {
        commandBus.send(reconcile(accountId, cmd));
    }

    private CloseBooks reconcile(TradingAccountId accountId, CloseBooks cmd) {
        if (cmd.accountId() == null) return new CloseBooks(accountId, cmd.nextPeriodId());
