...
/**
 * The read shape this slice serves — the current price of one instrument, and nothing else.
 * <p>
 * Returned straight from the API; there is no DTO between this and the wire (§R2).
 */
public record LatestPrice(InstrumentId instrumentId,
                          Amount latestPrice) {
    public LatestPrice {
        requireNonNull(instrumentId, "No instrumentId provided");
        requireNonNull(latestPrice, "No latestPrice provided");
    }
}
