...
/**
 * The application-property override for the trading-account closing-books policy.
 *
 * <p>Every field is nullable on purpose: {@code null} means "not overridden", and
 * {@code TradingAccountClosingBooksPolicy} then falls back to what the {@code @AggregateClosingBooksPolicy} annotation
 * on {@code TradingAccount} resolved to, and finally to a hardcoded default. A primitive or a defaulted field here
 * would silently win over the annotation.
 *
 * <p>Mutable with setters because Spring Boot binds it that way; it is not a domain type, which is why it lives in
 * {@code config/} rather than {@code types/}. The runtime-mutable shape of the same values is the immutable
 * {@code ClosingBooksSettings}.
 */
