...
/**
 * Record that the completed settlement has been reconciled against the books.
 *
 * <p>Both the command dispatched on the {@code CommandBus} and, when the endpoint builds it from its path variable,
 * the payload of {@code POST /api/admin/settlements/{settlementId}/reconciliation}.
 *
 * <p>Reconciliation cannot happen before the settlement has completed; repeating it is a no-op.
 */
public record ReconcileSettlement(SettlementId settlementId) {
