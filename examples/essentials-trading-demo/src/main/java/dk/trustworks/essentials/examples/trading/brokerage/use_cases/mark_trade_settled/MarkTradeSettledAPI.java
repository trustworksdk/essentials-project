...
/**
 * The API file of the {@code brokerage.mark_trade_settled} slice (rules/slice-design.md &sect;R2).
 * <p>
 * The command carries only the aggregate id, so the endpoint takes it as a typed {@link TradeId} path variable and
 * assembles the command inline -- assembly, which &sect;R2 allows, not a DTO.
 * <p>
...
public class MarkTradeSettledAPI {
    private final CommandBus commandBus;

    public MarkTradeSettledAPI(CommandBus commandBus) {
        this.commandBus = requireNonNull(commandBus, "No commandBus provided");
    }

    @PostMapping("/{tradeId}/settlement")
    public void markSettled(@PathVariable TradeId tradeId) {
