...
/**
 * The API file of the {@code brokerage.release_funds} slice (rules/slice-design.md §R2).
 *
 ...
 *
 * <p>Uses {@code send} rather than {@code sendAndDontWait} so a caller that releases and then reserves again sees the
 * released cash.
 */
@RestController
@RequestMapping(path = "/api/admin/trading-accounts")
public class ReleaseFundsAPI {
    private final CommandBus commandBus;

    public ReleaseFundsAPI(CommandBus commandBus) {
        this.commandBus = requireNonNull(commandBus, "No commandBus provided");
    }

    @PostMapping("/{accountId}/fund-releases")
    public void releaseFunds(@PathVariable TradingAccountId accountId,
                             @RequestBody ReleaseFunds cmd) {
        commandBus.send(reconcile(accountId, cmd));
    }

    private ReleaseFunds reconcile(TradingAccountId accountId, ReleaseFunds cmd) {
        if (cmd.accountId() == null) return new ReleaseFunds(accountId, cmd.amount());
