...
class NextGenCostDecompositionIT {
    private static final Logger log = LoggerFactory.getLogger(NextGenCostDecompositionIT.class);
    ...
    void decompose_the_cost_advantage_into_design_and_batching() throws Exception {
        ...
        var baselineWal = 1905.0d; // measured in NextGenVsBaselineCostIT under the same conditions
    ...
    private RunResult measure(Step step, int repetition, Map<String, String> environment) throws Exception {
        ...
        var settings = step.perMessageAck()
                       ? new ShardOwnerSettings(defaults.readBatchSize(), 1, Duration.ZERO,
                                                ...
                                                defaults.keyConcurrency(), defaults.pollBackstop(), Duration.ofSeconds(30), 512, 2, Duration.ofSeconds(5))
                       : defaults;

        var handled = new AtomicInteger();
        try (var queue = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "bench")) {
            queue.startConsuming(ignored -> handled.incrementAndGet(), settings, SHARD_COUNT);
            ...
            var table = "table." + NextGenSchema.UNORDERED_TABLE + ".";
