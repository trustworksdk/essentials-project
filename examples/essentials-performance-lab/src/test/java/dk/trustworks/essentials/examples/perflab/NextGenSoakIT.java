...
class NextGenSoakIT {
    private static final Logger log = LoggerFactory.getLogger(NextGenSoakIT.class);
    ...
    private List<Window> soakShardOwned(Duration duration, Duration window) throws Exception {
        ...
        try (var queue = new NextGenQueue(dataSource, QUEUE_ID, SHARD_COUNT, "soak")) {
            queue.startConsuming(payload -> {
            ...
            return run(duration, window, recorder, handled, NextGenSchema.UNORDERED_TABLE, intended -> {
