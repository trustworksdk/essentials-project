...
public class ShipOrderHandler extends AnnotatedCommandHandler {
    ...
    void handle(ShipOrder cmd) {
        ...
        if (existingOrder.markOrderAsShipped()) {
            // Spring Data MongoDB does no dirty checking - unlike JPA, a loaded document is not tracked, so the
            // mutation has to be written back explicitly or the idempotency guard never survives a redelivery
