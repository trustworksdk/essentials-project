...
import dk.trustworks.essentials.spring.examples.mongodb.messaging.shipping.entities.ShippingOrder;
import dk.trustworks.essentials.spring.examples.mongodb.messaging.shipping.entities.ShippingOrders;
import dk.trustworks.essentials.spring.examples.mongodb.messaging.shipping.events.ShippingOrderRegistered;
...
/**
 * The single decision component of the {@code shipping.register_shipping_order} slice (§R1).
 * <p>
 * This slice does the unpacking of its own command: neither {@link ShippingOrder} nor
 * {@link ShippingOrderRegistered} names {@link RegisterShippingOrder}, because a command type is slice-private and
 * {@code events/} is the bounded context's importable surface (§R4).
 */
@Service
public class RegisterShippingOrderHandler extends AnnotatedCommandHandler {
    ...
    void handle(RegisterShippingOrder cmd) {
        if (shippingOrders.findOrder(cmd.orderId()).isPresent()) {
            // Registering an order that already exists is a no-op, not an error - the command can be redelivered
            return;
        }
        ...
        eventBus.publish(new ShippingOrderRegistered(cmd.orderId(),
                                                     cmd.destinationAddress()));
    }
}
