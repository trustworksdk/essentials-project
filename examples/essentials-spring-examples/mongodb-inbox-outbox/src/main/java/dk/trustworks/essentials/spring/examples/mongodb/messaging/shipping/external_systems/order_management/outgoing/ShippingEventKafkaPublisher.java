...
package dk.trustworks.essentials.spring.examples.mongodb.messaging.shipping.external_systems.order_management.outgoing;
...
import dk.trustworks.essentials.spring.examples.mongodb.messaging.shipping.events.OrderShipped;
 ...
 *
 * <p>That combination is the point of the pattern. Because the {@code Outbox} write joins the same transaction as the
 * state change, the order is marked shipped and the outgoing message is recorded atomically: neither can happen
 * without the other. A separate consumer then forwards the message to the {@code shipping-events} topic and retries
 * until the broker acknowledges, which is what makes publishing survive a Kafka outage without a distributed
 * transaction.
 */
@Service
