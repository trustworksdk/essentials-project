...
/**
 * The {@code shipping.order_status} view slice reads the same collection the command slices write, so unlike the
...
public class OrderStatusIT extends AbstractIntegrationTest {
    ...
    void the_view_reports_shipped_status_immediately_after_the_commands_that_change_it() {
        ...
        assertThat(queries.findOrderStatusById(registeredOnly)).isPresent();
        assertThat(queries.findOrderStatusById(registeredOnly).get().isShipped()).isFalse();
        assertThat(queries.findOrderStatusById(shippedOrder).get().isShipped()).isFalse();

        commandBus.send(new ShipOrder(shippedOrder));

        assertThat(queries.findOrderStatusById(shippedOrder).get().isShipped()).isTrue();
        assertThat(queries.findOrderStatusById(registeredOnly).get().isShipped()).isFalse();
    }
    ...
    void the_view_filters_and_lists_over_the_read_model_it_owns() {
        ...
        assertThat(queries.findByShipped(true)).extracting(OrderStatusView::getId)
                                               .contains(shippedOrder)
                                               .doesNotContain(registeredOnly);
        assertThat(queries.findByShipped(false)).extracting(OrderStatusView::getId)
                                                .contains(registeredOnly)
                                                .doesNotContain(shippedOrder);
        assertThat(queries.findAllBy()).extracting(OrderStatusView::getId)
                                       .contains(registeredOnly, shippedOrder);
    }
