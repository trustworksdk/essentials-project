...
/**
 * A load-generation harness, <strong>not a domain slice</strong>. It used to be a {@code @Service} in {@code src/main},
 * which meant the running demo application started a 20-consumer {@code load-test} Inbox at boot. It is test scope now,
 * picked up by {@code TestApplication}'s component scan, and used only by {@code LoadOrderShippingProcessorIT}.
 */
@Service
public class LoadOrderShippingProcessor extends AnnotatedCommandHandler implements Lifecycle {
    ...
    private final        LoadTestShippingOrders shippingOrders;
    private              Inbox          inbox;

    private final AtomicInteger receivedRecreateShippingOrderView = new AtomicInteger(0);

    private volatile boolean started = false;

    public LoadOrderShippingProcessor(Inboxes inboxes,
                                      CommandBus commandBus,
                                      LoadTestShippingOrders shippingOrders) {
    ...
    public AtomicInteger getReceivedRecreateShippingOrderView() {
        return receivedRecreateShippingOrderView;
    }
