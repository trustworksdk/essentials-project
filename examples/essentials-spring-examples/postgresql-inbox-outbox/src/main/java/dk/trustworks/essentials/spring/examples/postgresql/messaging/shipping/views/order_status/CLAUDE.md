...
There is **one** table, written by the command slices and read here, so §R4's ownership rule is
restated rather than dropped:

> A view slice may read the entity's table, but **never through the write repository**.
...
- **Eventual consistency.** There is no projector. Reads hit the same table the command slices
...
## Boundaries
**Reads:** the `ShippingOrder` table, through `OrderStatusQueries` only.
**Forbidden:**
...
## Files
- `OrderStatusView.java` — the read shape; also the response body (§R2, no DTO). `getId()` is a
  `String` here because the entity's `@Id` is
