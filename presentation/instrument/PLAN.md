...
- `framework-snippets.md` records that these two excerpts are the single exception to
...
| 2. Extract snippets verbatim | done at the time, then removed: the copies duplicated sources three directories away in the same repository. Panels are trimmed excerpts of real code; the framework excerpts stay hand-maintained in `framework-snippets.md` |
| 3. Act 1 event-model strip | done — inline SVG on slide 4 |
| 4. Deck skeleton | done — 28 slides |
| 5. English content | done |
| 6. `demo-script.md` | written, **not yet executed** — run it end to end once before the talk |
...
1. Execute `demo-script.md` end to end from a wiped database.
...
presentation/instrument/
  ...
  framework-snippets.md  the framework's own examples, quoted from the LLM docs by hand
```

The module-landscape slide was planned to reuse the repository's `images/essentials-modules.png`; the deck as
built draws that content instead, and references no image at all.
...
**Fallback.** Talk through the slide behind a failing segment; no captures are kept in the repository. If
...
2. ~~Extract all snippets into a `snippets/` directory, verbatim from the real sources.~~ Removed later —
   the copies only duplicated sources in the same repository. Verify every file
