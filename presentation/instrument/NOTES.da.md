...
Driller et segment, så tal dig gennem sliden bagved og gå videre. Debug ikke på scenen. Der ligger ingen
optagelser i repositoryet — se `demo-script.md` § Fallbacks.
...
- [ ] kodepanelerne passer stadig til trading-demoen — decket citerer `market_data`'s
      `register_instrument`, `aggregates`, `types`, `config` og begge `views/`-slices; skim dem efter enhver
      refaktorering af demoen, og tjek `framework-snippets.md` mod de `LLM/`-dokumenter den citerer
- [ ] Fuld gennemkørsel mod et ur, mindst én gang, højt
- [ ] Demo kørt fra start til slut fra en slettet database
- [ ] Demoen kørt igennem fra en tømt database mindst én gang, med tidtagning
