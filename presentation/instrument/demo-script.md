...
No screen captures are kept in this repository. If a live segment misbehaves, talk through the slide and
move on — every segment's point is on the slide behind it, and debugging on stage costs more than the
segment is worth.

If you do want captures, record them while running segments A to C above from a wiped database, in the same
window layout you will present in, and keep them on the presenting machine. The Act 6 AI segment is the one
that is *always* a clip rather than a live run; keep that file with your own presenting material.
