...
If any segment misbehaves, talk through the slide behind it and move on. Do not debug on stage. No captures
are kept in the repository — see `demo-script.md` § Fallbacks.
...
**Slide 29, the AI segment.** Show the diff, play the clip from your own presenting material, and make only the narrow claim: the
...
- [ ] the code panels still match the trading demo — the deck quotes `market_data`'s
      `register_instrument`, `aggregates`, `types`, `config` and both `views/` slices; skim those after any
      refactor of the demo, and re-check `framework-snippets.md` against the `LLM/` docs it cites
- [ ] Full run against a clock, at least once, out loud
- [ ] Demo executed end to end from a wiped database
- [ ] Demo run end to end from a wiped database at least once, timed
