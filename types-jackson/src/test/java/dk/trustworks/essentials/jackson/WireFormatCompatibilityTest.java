...
class WireFormatCompatibilityTest {
    ...
    /** Only resolvable from the {@code types-jackson} module, which owns the file. */
    private static final Path GOLDEN_SOURCE = Path.of("src", "test", "resources", "wire-format",
                                                      "serialization-test-subject.json");

    private final ObjectMapper objectMapper = EssentialTypesJacksonModule.createObjectMapper(new Jdk8Module(),
                                                                                            new JavaTimeModule());
    ...
    private static void regenerate(String serialized) throws IOException {
        assertThat(Files.isDirectory(Path.of("src", "test", "java")))
                .as("Regenerate from the types-jackson module directory, which owns the golden document")
                .isTrue();
