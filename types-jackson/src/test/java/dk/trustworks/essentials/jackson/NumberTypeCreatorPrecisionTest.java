 ...
 * <p>
 * On this flavor the loss was not even conditional on that constructor — pre-fix, Jackson 2 narrowed a
 * high-precision decimal through a {@code double} whichever creator it picked, so the guarantee asserted here did not
 * hold for <em>any</em> shape. It was visible as a constructor-dependent difference only on Jackson 3, which is where
...
class NumberTypeCreatorPrecisionTest {
    ...
    @Test
    void test_the_natural_shape_preserves_a_decimal_beyond_double_precision() throws Exception {
        assertThat(objectMapper.readValue(HIGH_PRECISION, ValueTypedCreatorOnly.class).value())
                .isEqualByComparingTo(HIGH_PRECISION);
    }

    @Test
    void test_a_double_constructor_no_longer_costs_precision() throws Exception {
     ...
     */
    @Test
    void test_the_shipped_Amount_preserves_precision_despite_its_double_constructor() throws Exception {
    ...
    @Test
    void test_every_other_numeric_token_still_reads() throws Exception {
