...
- **`shardCount` can GROW**, via `ShardOwnedSchema.growShardCount(ds, name, n)` — refused while the ordered lane holds anything, and shrinking is never allowed. Restart instances afterwards; the count is baked into a running queue.
